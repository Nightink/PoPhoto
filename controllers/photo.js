...
exports.photos = function(req, res) {
    ...
    Photo.find(null, null, {limit: 15, skip: 0, sort: {updated: -1}}, function(err, docs){
        if(err) {
            console.log(err);
            return res.send(500);
        }
...
exports.photosJson = function(req, res) {
    ...
    var keywords = req.query.keywords && req.query.keywords.split(',');     //用户查询图片关键字
    ...
    var opt = {limit: limit, skip: skip, sort: {updated: -1}};
...
exports.addCommentsPhoto = function(req, res) {
    ...
    var update =  {"$push": {reviews: pushObj}, "$set": {update: new Date()}};
...
exports.poPhoto  = function(req, res) {
    ...
    reqPost.title = title.substring(0, title.lastIndexOf('.'));     //过滤文件后缀名
    reqPost.created = reqPost.updated = Date.now();
    reqPost.keywords = reqPost.keywords.split(/;|；|\s|,|，/);   //关键字处理
...
exports.updatePhoto = function(req, res) {
    ...
    Photo.findOne({_id: reqBody._id }, function(err, doc) {
        ...
        if(!_.isArray(reqBody.keywords)) {
            doc.keywords = reqBody.keywords.split(/;|；|\s|,|，/);   //关键字处理
        }
